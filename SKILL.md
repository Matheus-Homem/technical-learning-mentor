...
```
.mentor/
  .gitignore                    # ignores progress.html and every features/*/classes/
...
`scripts/md-to-audio/` holds the narration pipeline used by `/mentor-class` when the chosen format includes audio: `prepare_narration.py`, `generate_audio.py`, and `narration_glossary.json`. These are version-controlled as part of the skill; their output is not. The procedure, the prerequisites, and two warnings worth reading before the first run are in `references/audio.md`.
