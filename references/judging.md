...
- **Multiple choice**: distractors must be plausible wrong models, ideally misconceptions already recorded for this user. Four options. Never use "all of the above". Remember MCQ can only support `explains`.
- **Short answer**: ask for the mechanism or the reason, not the definition. "Why does X exist?" beats "What is X?".
- **Scenario**: state a condition change and ask what they would do and what breaks if they don't. This is the only written format that supports `decides`.
- **Prediction**: asked before the user runs something. "What do you expect to happen, and what would tell you your model is wrong?"
